...
const UserProfile: React.FC = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      // If there's a new image, append it, otherwise use existing image URL
      if (fileList.length > 0 && fileList[0].originFileObj) {
        formData.append("image", fileList[0].originFileObj);
      } else {
        formData.append("imageUrl", profileData?.imageUrl || ""); // Fallback for existing image URL
      }
      ...
      if (updatedProfile) {
        ...
        setIsModalVisible(false);
        setProfileData(updatedProfile); // Update the state with the updated profile
        form.resetFields();
        setFileList([]);
      }
    } catch (error) {
